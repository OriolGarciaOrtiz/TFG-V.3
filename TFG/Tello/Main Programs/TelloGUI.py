    ...
    def update_frame(self):
        ...
        if self.controller.is_connected:
            ...
            if object_center and distance is not None:
                ...
                if self.opt_cam.get() == "Drone Camera":
                    self.controller.yaw_velocity = int(np.clip(speed_x, -self.max_velocity.get(), self.max_velocity.get()))
                    self.controller.up_down_velocity = int(np.clip(speed_y, -self.max_velocity.get(), self.max_velocity.get()))
                    self.controller.for_back_velocity = int(np.clip(speed_z, -self.max_velocity.get(), self.max_velocity.get()))
                else:
                    self.controller.left_right_velocity = int(np.clip(speed_x, -self.max_velocity.get(), self.max_velocity.get()))
                    self.controller.for_back_velocity = int(np.clip(speed_y, -self.max_velocity.get(), self.max_velocity.get()))
                    self.controller.up_down_velocity = 0
