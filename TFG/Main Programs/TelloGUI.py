...
class GUI:
    ...
    def create_hsv_sliders(self):
        ...
        self.horse_img = load_img("Fotos/horse.png")
        self.bird_img = load_img("Fotos/bird.png")
        self.dog_img = load_img("Fotos/dog.png")
        self.cat_img = load_img("Fotos/cat.png")
    ...
    def execute_drone_action(self, action: str):
        ...
        # Evita activar varias veces la misma acción durante el tiempo de offset
        if self.action_active:
            ...
            # Reset antes de aplicar uno nuevo
            self.reset_offsets()

            # Asignar offsets según acción
            if action == "Move Up":
                self.offset_ud = +40
                ...
                # Caso especial: bloquear PID → dejar en 0 y lanzar land
                self.block_PID = True
    ...
    def detect_objects_color_contour_dual_gamemode(self, mask_green, mask_orange, mask_blue, mask_purple, img_contour):
        kernel = np.ones((5, 5), np.uint8)
        def get_largest_contour(mask):
            mask_clean = cv2.erode(mask, kernel, iterations=2) #Probar tamiben hacer lo de canny y toda la otra parte para el tema de colores
            mask_clean = cv2.dilate(mask_clean, kernel, iterations=3)
            contours, _ = cv2.findContours(mask_clean, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
            if contours:
                largest = max(contours, key=cv2.contourArea)
