...
cv2.createTrackbar("HUE Min", "HSV", 53, 179, empty)
...
#Detección de distancia con ArUco

# --- Diccionario ArUco y parámetros ---
aruco_dict = cv2.aruco.getPredefinedDictionary(cv2.aruco.DICT_5X5_100)
parameters = cv2.aruco.DetectorParameters()

# Crear detector (nuevo estilo API)
...
marker_length = 0.06       # Lado real del marcador ArUco (m)
desired_distance = 0.7       # Distancia deseada al objeto (m)
W_real = 0.20                # Ancho real del objeto a seguir sin marcador (m)
f = camera_matrix[0, 0]      # Distancia focal efectiva en píxeles

# --- MODO DE OPERACIÓN ---
# 0 = Usar marcador ArUco
# 1 = Usar detección de contorno (sin marcador)
modo = 0
...
while True:
    ...
    # --- MODO 0: detección con marcador ArUco ---
    if modo == 0:
        corners, ids, rejected = detector.detectMarkers(gray)
        if ids is not None:
            cv2.aruco.drawDetectedMarkers(imgContour, corners, ids)
            ...
            for i in range(len(ids)):
                ...
                # Calcular la pose con solvePnP
                ret, rvec, tvec = cv2.solvePnP(obj_points, corners[i], camera_matrix, dist_coeffs)

                # Dibujar ejes 3D
                cv2.drawFrameAxes(imgContour, camera_matrix, dist_coeffs, rvec, tvec, 0.1)

                # Extraer distancia (Z)
                distance = float(tvec[2])
                print(f"[ArUco] Distancia al marcador {ids[i][0]}: {distance:.2f} m")

        else:
            me.for_back_velocity = 0

    # --- MODO 1: detección de contorno (sin marcador) ---
    elif modo == 1:
        contours, _ = cv2.findContours(imgDil, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
        if contours:
            ...
            # Calcular distancia estimada con la fórmula de pinhole
            distance = (f * W_real) / w
            print(f"[Contorno] Distancia estimada: {distance:.2f} m")

        else:
            me.for_back_velocity = 0

    # --- control con PID ---
    speed_yaw, speed_ud,speed_fb = 0, 0,0
    if object_center is not None:
        ...
        if valor == 0:  # Modo Ziegler–Nichols (solo Kp bajo, medir Tu)
            ...
            speed_fb=int(Kpz * error_z + Kiz * integral_z + Kdz * derivative_z)
    ...
    if not simulation:
        ...
        print(f"Simulación → yaw: {speed_yaw}, up/down: {speed_ud} ,for/back: {speed_fb}")

    # --- mostrar todas las imágenes en una sola ventana ---
    stack = stackImages(0.7, ([img, result, mask_bgr],
